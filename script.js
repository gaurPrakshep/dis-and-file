...
function initAddButtons(){
  document.querySelectorAll(".add-btn").forEach(btn => {
    btn.addEventListener("click", () => {
      ...
      // Get the original price from dropdown
      const originalPrice = Number(val);
      const sizeLabel = select.options[select.selectedIndex]?.getAttribute("data-size") || select.options[select.selectedIndex]?.text || "";

      // Get discount from .price-list (if available)
      const priceList = item.querySelector('.price-list');
      const discount = priceList ? parseFloat(priceList.dataset.discount) || 0 : 0;
      ...
      // Add discounted price to cart
      addToCart(itemName, sizeLabel, Number(discountedPrice), qty);
       showNotification(`${itemName} (${sizeLabel}) added to cart with ${discount}% OFF!`, "success");
    });
  });
} 
...
// Checkout
// ✅ Enhanced checkout handler with Google Sheets grouped order logging
checkoutBtn.addEventListener("click", () => {
  ...
  // ✅ Build order rows (one per item)
  const orderRows = cart.map((it, idx) => ({
    datetime: dateTime,
    name: it.name,
    size: it.size,
    qty: it.qty,
    price: it.price,
    total: idx === 0 ? total : "" // only first item shows total
  }));

  // ✅ Send to Google Apps Script endpoint
  fetch("https://script.google.com/macros/s/AKfycbwbQhzMbGxEIXrmtjOYctuw5uuxfw0xkJIjld3fE8TJQ8Q15-2d_9t6zKoITSF8tnYA/exec", {
    method: "POST",
    mode: "no-cors",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ order: orderRows })
  })
  .then(() => console.log("✅ Order sent to Google Sheet"))
  .catch(err => console.error("❌ Error sending order:", err));

  // Continue WhatsApp flow (unchanged)
  checkoutToWhatsApp();
});
...
// Quantity control functions (adds +/- for easier changes on mobile)
function ensureQuantityControls() {
  document.querySelectorAll('.qty-input').forEach((input) => {
    // If already wrapped, skip
    if (input.parentElement && input.parentElement.classList.contains('qty-controls')) return;
    ...
    dec.textContent = '−';
    dec.addEventListener('click', () => changeQuantity(input, -1));

    const inc = document.createElement('button');
    inc.type = 'button';
    inc.className = 'qty-btn qty-increase';
    inc.textContent = '+';
    inc.addEventListener('click', () => changeQuantity(input, 1));
...
function changeQuantity(inputOrButton, delta) {
  const input = inputOrButton.classList?.contains('qty-input')
    ? inputOrButton
    : inputOrButton.closest('.qty-controls')?.querySelector('.qty-input');
  if (!input) return;
  const current = parseInt(input.value || '1', 10);
  const next = Math.max(1, isNaN(current) ? 1 + delta : current + delta);
...
function validateQuantity(input) {
  if (Number(input.value) < 1 || input.value === '' || input.value === '0') {
    input.value = 1;
...
document.querySelectorAll('.price-list').forEach(list => {
  const discount = parseFloat(list.dataset.discount) || 0;
  list.querySelectorAll('p').forEach(p => {
    const text = p.textContent.split('–')[0].trim();
    const oldPrice = parseFloat(p.querySelector('.old-price').textContent.replace('₹',''));
    const newPrice = (oldPrice * (1 - discount / 100)).toFixed(0);
    p.querySelector('.new-price').textContent = `₹${newPrice}`;
  });
});
